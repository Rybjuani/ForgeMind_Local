...
@dataclass
class DemoModelConfig(ModelConfig):
    """ModelConfig override used when MOCK_LLM=1 is set.

    Lets us inject fixed ``quant`` / ``size_human`` / ``size_bytes``
    values that match the v14 mockup ("Gemma 4 12B · Q4_K_M · 7.4 GB")
    without needing a real .gguf on disk. The ``gguf_path`` carries a
    realistic-looking path (matches the mockup) but ``exists()`` lies
    so the config screen shows "OK" status; the backend's
    ``_resolve_mode`` short-circuits to mock mode via the
    ``is_demo`` flag so no real subprocess is spawned.
    """

    mock_quant: str = ""
    mock_size_human: str = ""
    mock_size_bytes: int = 0
    is_demo: bool = True

    @property
    def quant(self) -> str:  # type: ignore[override]
        return self.mock_quant

    @property
    def size_bytes(self) -> int:  # type: ignore[override]
        return self.mock_size_bytes

    @property
    def size_human(self) -> str:  # type: ignore[override]
        return self.mock_size_human

    def exists(self) -> bool:  # type: ignore[override]
        # Demo config is "loaded" — the config screen shows OK status
        # even though the .gguf file is not on disk. The backend's
        # _resolve_mode checks `is_demo` and skips the real subprocess.
        return True